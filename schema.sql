...
create table
  public.users (
    ...
    constraint users_name_check check ((length(name) <= 20))
...
create table
  public.animals (
    ...
    constraint animals_name_key unique (name),
    ...
    constraint animals_health_check check (
      (
        (health >= '-1'::integer)
        and (health <= 101)
      )
    ),
    constraint animals_name_check check ((length(name) <= 20))
  ) tablespace pg_default;
