...
You should use "generated always" instead of "generated by default" when creating primary keys/ID's to ensure that they're always unique. This would prevent users from ever being able to insert ID's for new rows that match ID of already existing rows.
...
You have a lot of columns that use nullable values. I'd suggest reviewing this and changing some to not null, like names in users and foreign key references.
...
Tables only allow users to have one animal. It may be useful to add a table to keep inventory of the animals the user had purchased, like an animal ledger.
We are currently implementing only one animal for a user.
In inventory, you may want to return a different value for animal when it doesn't exist instead of -1. Maybe a message to the user.
...
I would change create-user to user, as POST already implies the creation of a new resource on the server. Same with create-animal
We think it’s more clear to write create because we have create and buy functions for both animals and users.
Animals table and users table have circular dependency. I'd recommend removing one reference.
...
Allows for negative attack and defense values in create-animal, results in negative price and will cause unexpected results
We have fixed that by adding the appropriate checks.
May consider implementing checks to prevent creation of animals with the same name
We have fixed it where no animal can have the same name
Returns true when buying animals with names that don't match their id
We have removed the animal_name part so the buy_animal function only accepts user and animal id’s
Restock endpoint is returning 500 errors with expected input. This happens when the user doesn't own an animal but you may want to return a prompt to buy an animal instead of error.
Fixed it where if a user doesn't own an animal, the message of “Unable to restock. You don't own an animal” pops up
create-animal has input field price in API specs but not in docs
We have fixed it to be consistent with the code
Was able to create animal with attack and defense values outside of range 1-50, as specified in API specs
...
Audit endpoint requests user_id, but says username in API specs
